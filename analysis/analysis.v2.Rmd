...
Notes:
  1) we multiply the percentage numbers by 10 to take into account the 10% sample
  1) not standardised
...
cat( "state variation")
df_patient %>% 
  filter( user_type=='2_double-dipper') %>%
  count( state ) %>%
  f_join_population( c('state'), df_population. = df_population_2015) %>%
  mutate( pct = round( n*10 / population, 10 )) %>%
  kable()
...
df_patient %>% 
  ...
  mutate( pct = round( n*10 / population*100, 10 )) %>%
  kable()
